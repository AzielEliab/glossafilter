...
<body>
  ...
  <div id="meshStrip" aria-label="Suite Live Nodes">
    ...
    <div>
      ...
      <button id="meshJoin" type="button" title="Join as glossafilter. Refused while mesh is OFF. No auto-join.">Join</button>
      <button id="meshLeave" type="button" title="Leave this node. No auto-heal.">Leave</button>
    </div>
    <p id="meshProducts">Catalog MCP mesh_* · FragGate slug=mesh · /v1/mesh/* PROXY · QNS-CD-1.0 photon QNS1 · not a public qnsd proxy · not AnonBroadcast · not AZMail ring · not a Node Gate</p>
  </div>
  <div class="card">
    <div class="nums">
    ...
    <script>
      ...
      (function () {
        ...
        function paintMesh(raw) {
          ...
          $("meshProducts").textContent = "Catalog MCP mesh_* · FragGate slug=mesh · /v1/mesh/* PROXY · QNS-CD-1.0 photon QNS1 · not a public qnsd proxy · not AnonBroadcast · not AZMail ring · not a Node Gate" + extra;
        }
        ...
        async function refreshMesh() {
          try {
            ...
            paintMesh(merged);
            var nodeId = sessionStorage.getItem("glossafilter_mesh_node");
            if (on && nodeId) {
        ...
        $("meshDisable").onclick = async function () {
          sessionStorage.removeItem("glossafilter_mesh_node");
          paintMesh(await meshPost("/v1/mesh/disable", {}));
          refreshMesh();
        };
        $("meshJoin").onclick = async function () {
          var j = await meshPost("/v1/mesh/join", { product: "glossafilter", label: "Glossa Filter Worker" });
          var inner = unwrapMesh(j);
          var id = inner.node_id || inner.id || (inner.session && inner.session.node_id);
          if (id) sessionStorage.setItem("glossafilter_mesh_node", String(id));
          paintMesh(j);
          refreshMesh();
        };
        $("meshLeave").onclick = async function () {
          var id = sessionStorage.getItem("glossafilter_mesh_node");
