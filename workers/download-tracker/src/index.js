 ...
 *
 * GET  /download?repo=AzielEliab/glossafilter&tag=latest&asset=...
 ...
 * Isolated counter: Worker glossafilter-download-tracker, project glossafilter.
 * Not mixed with any other product.
 */

const PROJECT = "glossafilter";
const DEFAULT_ASSET = "glossafilter-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "glossafilter";
const DEFAULT_BRANCH = "main";
const GITHUB_RELEASES = "https://github.com/AzielEliab/glossafilter/releases";
const GITHUB_LATEST = "https://github.com/AzielEliab/glossafilter/releases/latest";
const HOST = "https://glossafilter-download-tracker.vibelock.workers.dev";
const GITHUB_REPO = "https://github.com/AzielEliab/glossafilter";
const INSTALL_LINE = "curl -fsSL https://glossafilter-download-tracker.vibelock.workers.dev/install.sh | bash";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 Glossa Filter-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0;
  let forks = 0;
  let watchers = 0;
  let release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/glossafilter", { headers });
    if (repoRes.ok) {
    ...
    const relRes = await fetch("https://api.github.com/repos/AzielEliab/glossafilter/releases/latest", { headers });
    if (relRes.ok) {
...
# Glossa Filter one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
...
<title>Glossa Filter — Aziel Eliab</title>
<meta name="description" content="Deterministic linguistic mediation by Aziel Eliab that renders the same intent into peer languages and dialects.">
<meta name="author" content="Aziel Eliab">
<link rel="canonical" href="https://glossafilter-download-tracker.vibelock.workers.dev/">
<meta property="og:title" content="Glossa Filter — Aziel Eliab">
<meta property="og:description" content="Deterministic linguistic mediation by Aziel Eliab that renders the same intent into peer languages and dialects.">
<meta property="og:url" content="https://glossafilter-download-tracker.vibelock.workers.dev/">
...
<script type="application/ld+json">
{
  ...
  "codeRepository": "https://github.com/AzielEliab/glossafilter",
  "downloadUrl": "https://glossafilter-download-tracker.vibelock.workers.dev/download",
  "license": "https://www.apache.org/licenses/LICENSE-2.0",
  "url": "https://glossafilter-download-tracker.vibelock.workers.dev/",
  "description": "Deterministic linguistic mediation by Aziel Eliab that renders the same intent into peer languages and dialects."
}
</script>
<!-- gitbaby-seo -->
<style>
...
<body>
  ...
  <div class="card">
    ...
    <p class="kid"><strong>Two big buttons.</strong> Download saves the gzip (the Downloads number goes up). One-click install copies a Terminal command. After it finishes, type <code>glossafilter ui</code>.</p>
    <div class="btns">
      <a class="btn primary dl" href="/download?asset=${DEFAULT_ASSET}">Download</a>
      <button type="button" class="btn install" id="install-btn">One-click install</button>
    </div>
    <pre id="install-cmd">${INSTALL_LINE}</pre>
    <p class="kid">Then run: <code>glossafilter ui</code> and open http://127.0.0.1:8792 (this computer only).</p>
    ...
    <p class="iso">Isolated counter: Worker <code>glossafilter-download-tracker</code>, project <code>${PROJECT}</code>, KV <code>GLOSSAFILTER_DOWNLOADS</code>. Not mixed with any other product. /v1 does not increment downloads.</p>
    ...
    <script>
      (function () {
        var cmd = "curl -fsSL https://glossafilter-download-tracker.vibelock.workers.dev/install.sh | bash";
        var btn = document.getElementById("install-btn");
        var pre = document.getElementById("install-cmd");
        if (!btn) return;
        btn.addEventListener("click", function () {
          function done(ok) {
            btn.textContent = ok ? "Copied! Paste in Terminal, then run glossafilter ui" : "Select the command, copy it, then run glossafilter ui";
...
<section class="cite" id="cite">
  <h2>How to cite</h2>
  <p>Aziel Eliab. Glossa Filter. https://github.com/AzielEliab/glossafilter. https://glossafilter-download-tracker.vibelock.workers.dev.</p>
  <p><a href="https://aziel-runtime.vibelock.workers.dev/">Catalog</a> · <a href="https://github.com/AzielEliab/glossafilter">GitHub</a> · <a href="https://glossafilter-download-tracker.vibelock.workers.dev/download">Download</a> · <a href="https://glossafilter-download-tracker.vibelock.workers.dev/cite.json">cite.json</a></p>
</section>
<!-- /gitbaby-seo -->
</body>
</html>`;
...
export default {
  async fetch(request, env) {
    ...
    if ((url.pathname === "/cite.json" || url.pathname === "/cite.json/") && request.method === "GET") {
      return json({"author": "Aziel Eliab", "title": "Glossa Filter", "github": "https://github.com/AzielEliab/glossafilter", "download": "https://glossafilter-download-tracker.vibelock.workers.dev/download", "doi": null, "license": "Apache-2.0", "catalog": "https://aziel-runtime.vibelock.workers.dev/"});
    }
    // /gitbaby-seo-routes
